...
void sEvaluator::ScoreHanging(sPosition *p, int side)
{
 ...
 int sEvaluator::ScorePatterns(sPosition *p, int side)
 {
	 int score = 0;
	  ...
	  if  ( ( bbPc(p, side, N) & SqBb(relativeSq[side][C3]) )
...
int sEvaluator::EvalFileStorm(U64 bbOppPawns, int side)
{
	...
	return 0; // TEMPORARY, I HOPE
	if (bbOppPawns & rank4[side]) return -5;
	if (bbOppPawns & rank5[side]) return -10;
	if (bbOppPawns & rank6[side]) return -15;
    return 0;
}
